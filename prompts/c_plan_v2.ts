...
語氣鐵律：
1. 犀利一針見血——好就說好，問題直接戳，不繞彎
2. 每句話都要有資訊量——砍掉「你的感受是合理的」「這很正常」等套話
3. 術語白話翻譯——先講結論，術語用括號附註
4. 鏡映開場——「你是不是常常...」精準描述客戶真實體驗（只在第一章命格名片用一次，其他章節不重複）
5. 壞消息不包糖衣——直說問題，但每條必附出路
6. 禁止宿命論——「命盤顯示傾向，你可以選擇」
7. 禁止任何形式的評分或分數——命不該有分數
8. 心理學只在「給你的一句話」和「刻意練習」兩處使用，其餘章節不出現心理學語言
9. 語言：${lang}
...
字數要求：本 Call 輸出 10000-14000字。四個章節深度均勻。`
}
...
### 你的感情模式
你在親密關係中是什麼樣的人——怎麼愛、怎麼被愛、最容易在哪裡卡住。
...
這是整份報告最能打中客戶的章節——讓他回顧過去時感嘆「太準了」，展望未來時有方向感。

### 過去 10 年回顧
逐步大運分析，精準回顧人生重大轉折。
- 每步大運：年份範圍 + 主題 + 「你那時候是不是...」的精準描述
- 交叉引用：八字大運 + 紫微大限 + 西洋占星行運 + 吠陀 Dasha + 七政推運
- 不要泛泛而談，要精準到「XX年你是不是經歷了...」
...
字數要求：本 Call 輸出 9000-13000字。四個章節深度均勻。`
}
...
把整份報告的洞見轉化為客戶可以立刻執行的行動。

這是唯一允許心理學包裝的地方——每個練習用心理學解釋「為什麼有效」，用命理解釋「為什麼適合你」。
...
每一項必須包含：
1. **練什麼**（具體命名）
2. **為什麼適合你**（命格依據，精確到天干地支/星曜/卦象）
3. **怎麼練**（具體步驟，至少3步）
4. **為什麼有效**（心理學原理，白話文包裝——這裡允許心理學語言）
5. **會遇到什麼困難**（命格中的阻力）
6. **多久見效**（結合大運/流年，給具體時間）
...
字數要求：本 Call 輸出 5000-8000字。三個章節完整呈現。
...
export function buildAppendix(
  ...
  appendix += `\n### 排盤原始數據\n\n`
  appendix += `以下為各系統的排盤計算結果，供進階讀者參考。\n\n`

  for (const a of analyses) {
    appendix += `#### ${a.system}\n\n`

    if (a.sub_summary) {
      appendix += `**摘要**：${a.sub_summary}\n\n`
    }
    ...
    if (a.tables?.length) {
      for (const t of a.tables) {
        appendix += `**${t.title}**\n\n`
        if (t.headers) {
          appendix += `| ${t.headers.join(' | ')} |\n`
          appendix += `| ${t.headers.map(() => '---').join(' | ')} |\n`
        }
        if (t.rows) {
          for (const row of t.rows) {
            appendix += `| ${row.join(' | ')} |\n`
    ...
    if (a.good_points?.length) {
      appendix += `**好的地方**\n`
      for (const g of a.good_points) appendix += `- ${g}\n`
      appendix += `\n`
    }
    if (a.bad_points?.length) {
      appendix += `**需要注意**\n`
      for (const b of a.bad_points) appendix += `- ${b}\n`
      appendix += `\n`
    }
  }
