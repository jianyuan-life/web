...
## 【嚴格限制】E2 月盤出門訣只使用以下系統
本報告只能引用以下四個系統的排盤數據，其餘系統一概不得出現在報告中：
1. **奇門遁甲**（核心）：月盤方位、吉門吉神、格局吉凶、九遁
2. **八字**：用神、五行、流月運勢
3. **生物節律**：吉凶日、臨界日、高潮低潮週期
4. **風水八宅**：方位吉凶參考

禁止引用的系統（即使排盤數據中有也不得使用）：姓名學、人類圖、吠陀占星、西洋占星、南洋術數、數字能量、紫微斗數、生肖、星座。
如果排盤數據中只有上述四個系統以外的數據，則只分析奇門遁甲，其餘跳過。
...
**改善建議聚焦在：出行方位選擇、吉時利用、補運操作、五行養生。不要出現姓名改運、人類圖策略等無關內容。**
...
語言：繁體中文。
字數：3000-5000字（精簡實用，不灌水）。
核心原則：所有分析只基於奇門遁甲、八字、生物節律、風水八宅四個系統的排盤數據。這是「月度行動日曆」，打開就知道怎麼安排。
