...
const SPECIAL_STROKES: Record<string, number> = {
  ...
  '鵬':19,
  // 常見繁簡對照（簡體轉繁體後查筆畫，僅新增不重複的）
  '時':10,'鐘':20,'車':7,'魚':11,'鳥':11,'門':8,
  '畫':12,'會':13,'開':12,'關':19,'見':7,
  '話':13,'語':14,'說':14,'讀':22,'寫':15,'點':17,
  '動':11,'勞':12,'當':13,'實':14,'與':14,'電':13,'飛':9,
  '銀':14,'鐵':21,'鋼':16,'銅':14,
  '醫':18,'藥':19,'體':23,'頭':16,'臉':17,'齒':15,
  '連':14,'運':16,'進':15,'達':16,'遠':17,'邊':22,
  '陽':17,'陰':16,'雙':18,'單':12,'戰':16,
  '歲':13,'歷':16,'歡':22,'觀':25,'護':21,'響':22,
}
...
function getStrokes(char: string): number {
  ...
  // 查不到的字記錄下來，用常見筆畫 10 作為安全預設值
  // 不再用 Unicode 亂算，避免出現明顯錯誤的筆畫
  unknownChars.push(char)
  return 10 // 安全預設值，後續提示用戶確認
}
